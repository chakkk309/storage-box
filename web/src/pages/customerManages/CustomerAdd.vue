    ...
    >
      ...
      >
        <el-form-item label="客户名" prop="userName">
          <el-input v-model="addInfo.userName" autocomplete="off" class="secPass"></el-input>
        </el-form-item>
...
export default {
  data() {
    return {
      dialogadd: false,
      addInfo: { name: "", email: "", phone: "", userName: "", sex: "" },
      rules: {
        userName: [
          {
            required: true,
    ...
    addData(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          this.$axios
            .post("prod-api/backend/customer/create", pamars)
            .then(res => {
