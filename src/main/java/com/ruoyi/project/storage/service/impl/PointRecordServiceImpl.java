...
public class PointRecordServiceImpl implements IPointRecordService {
     ...
     */
    @Override
    public List<PointRecord> selectRecordList(Long userId) {
        // 实例化积分记录对象
        PointRecord pointRecord = new PointRecord();
        // 设置用户id
        pointRecord.setUserId(SecurityUtils.getUserId());
        // 返回积分记录集合
     ...
     */
    @Override
    public int insertPointRecord(RecordPointWay recordPointWay, Long userId, Long points,
                                 Long advertisementId, Long orderId) {
        // 实例化对象
        PointRecord pointRecord = new PointRecord();
        // 如果为手机注册赠送
